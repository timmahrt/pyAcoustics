...
def _intensityMorph(
    ...
    for fromTuple, toTuple in zip(fromDataTupleList, toDataTupleList):
        ...
        normFactorListTmp, a = getRelativeNormalizedFactors(
            fromSubWav, toSubWav, coreChunkSize
        )
        tmpChunkList = [tmpChunkSize for value, tmpChunkSize in normFactorListTmp]
    ...
    interpolatedResults = []
    normFactorGen = [
        sequences.interp(1.0, factor[0], numSteps) for factor in normFactorList
    ]
    ...
    for i in xrange(numSteps):
        ...
        for fromTuple, chunkSize in zip(fromDataTupleList, chunkSizeList):
            ...
            assert len(fromSubWav.rawDataList) == len(
                expandedNormFactorList[offset : offset + chunkSize]
            )

            tmpList = [
                fromSubWav.rawDataList,
                expandedNormFactorList[offset : offset + chunkSize],
            ]
            subRawDataList = [
                value * normFactor
                for value, normFactor in utils.safeZip(tmpList, enforceLength=True)
            ]
...
def getRelativeNormalizedFactors(fromDataList, toDataList, chunkSize):
    ...
    tmpIndexList = sequences.interp(0, len(toDataList) - 1, fromDataList)
    newTargetRawDataList = [toDataList[int(round(i))] for i in tmpIndexList]

    assert len(fromDataList) == len(newTargetRawDataList)

    fromGen = sequences.subsequenceGenerator(
        fromDataList, chunkSize, sequences.sampleMiddle, sequences.DO_SAMPLE_GATED
    )
    toGen = sequences.subsequenceGenerator(
...
def expandNormalizationFactors(normFactorList):
    ...
    while i < len(normFactorList) - 1:
        ...
        controlPoints.extend(my_math.linspace(startVal, startVal, chunkSize - 1))
        i += 1
    ...
    controlPoints.extend(my_math.linspace(startVal, startVal, finalChunkSize - 1))
