...
def _runSpeechRateEstimate(
    wavPath, syllableNucleiPath, matlabEXE, matlabScriptsPath, printCmd=True
):
    uwe_sr.findSyllableNuclei(
        wavPath, syllableNucleiPath, matlabEXE, matlabScriptsPath, printCmd
    )
...
def _runSpeechRateEstimateOnIntervals(
    ...
    uwe_sr.findSyllableNuclei(
        wavTmpPath, syllableNucleiPath, matlabEXE, matlabScriptsPath, printCmd
    )


def _addSyllableNucleiToTextgrids(
    wavPath, tgPath, tierName, syllableNucleiPath, outputPath
):
...
def _calculateSyllablesPerSecond(wavPath, syllableNucleiPath):
    ...
    for name in utils.findFiles(wavPath, filterExt=".wav", stripExt=True):
        ...
        nucleusSyllableList = [
            nucleus for subList in nucleusSyllableList for nucleus in subList
        ]
        numSyllables = len(nucleusSyllableList)
        wavFN = join(wavPath, name + ".wav")
        duration = audio_scripts.getSoundFileDuration(wavFN)

        print("%s - %.02f syllables/second" % (name, numSyllables / float(duration)))


def _calculateSyllablesPerSecondForIntervals(
    wavPath, tgPath, tierName, syllableNucleiPath
):
    # Add syllable nuclei to textgrids
    for name in utils.findFiles(wavPath, filterExt=".wav", stripExt=True):
        ...
        print(
            "%s - %s (syllables/second for each interval)"
            % (name, ",".join(durationList))
        )


def markupTextgridWithSyllableNuclei(
    wavPath,
    tgPath,
    tierName,
    wavTmpPath,
    ...
    _runSpeechRateEstimateOnIntervals(
        wavPath,
        tgPath,
        tierName,
        wavTmpPath,
    ...
    _addSyllableNucleiToTextgrids(
        wavPath, tgPath, tierName, syllableNucleiPath, outputPath
    )

    _calculateSyllablesPerSecondForIntervals(
        wavPath, tgPath, tierName, syllableNucleiPath
    )
...
if __name__ == "__main__":
    ...
    _matlabScriptsPath = "/Users/tmahrt/Dropbox/workspace/pyAcoustics/" "matlabScripts"
